...
- **Enums de APAC**: `priority` = `URGENTE` | `NORMAL`; `status` = `PENDENTE` → `AGUARDO` → `APROVADO` | `CANCELADO` | `NEGADO`. `status` **não** é enviado na criação. `acs` (Agente Comunitário de Saúde) é uma lista fixa de 14 pessoas, **obrigatória** em `POST /apacs` — não deriva de nenhum outro endpoint, é hardcoded em `APAC_ACS` mesmo. Todos já modelados em `src/lib/apac.ts` como `const object` + union type (`erasableSyntaxOnly` proíbe `enum`). `procedure` **não é mais um enum**: a API aceita texto livre (`ApacProcedure` é só um alias de `string`) — nos formulários é um `Input`, não um `Select`.
