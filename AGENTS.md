...
- **Enums de APAC**: `procedure` = `EXAME` | `CIRURGIA`; `priority` = `URGENTE` | `NORMAL`; `status` = `PENDENTE` → `AGUARDO` → `APROVADO` | `CANCELADO` | `NEGADO`. `status` **não** é enviado na criação. `acs` (Agente Comunitário de Saúde) é uma lista fixa de 14 pessoas, **obrigatória** em `POST /apacs` — não deriva de nenhum outro endpoint, é hardcoded em `APAC_ACS` mesmo. Todos já modelados em `src/lib/apac.ts` como `const object` + union type (`erasableSyntaxOnly` proíbe `enum`).
- **Edição e exclusão de APAC**: `PATCH /apacs/:id` (`name`/`acs`/`procedure`/`municipality`, todos opcionais — envie só o que mudou) e `DELETE /apacs/:id` (204, sem 404 — id inexistente cai em 500). `cns`, `priority`, `birth_date`, `cpf` e `status` não são editáveis por esse endpoint; transição de status continua sem endpoint. Implementado em `updateApac`/`deleteApac` (`src/api/apacs.ts`), `useUpdateApac`/`useDeleteApac` (`src/hooks/use-apacs.ts`), `ApacEditDialog`/`ApacDeleteAlert` (`src/components/apacs/`).
...
- `GET /apacs`, `POST /apacs` e `PATCH /apacs/:id` devolvem `cns`/`cpf` em **texto puro** (não são mais criptografados). Mesmo assim, os dois ficam de fora do view model `Apac`: o `PATCH` de edição não altera `cns` nem `cpf` (só `name`/`acs`/`procedure`/`municipality`), então nenhuma tela precisa deles — continuam dado pessoal de saúde e não devem ser exibidos em tabela, detalhe, busca, log ou `localStorage`.
