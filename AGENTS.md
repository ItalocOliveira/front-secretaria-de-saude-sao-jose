...
- **Enums de APAC**: `priority` = `URGENTE` | `ELETIVO` (`ELETIVO` era `NORMAL` — renomeado por regra de negócio; rótulo em tela já era "Eletivo", mas o valor enviado no `POST`/`PATCH` precisou acompanhar); `status` = `PENDENTE` → `AGUARDO` → `APROVADO` | `CANCELADO` | `NEGADO`. `status` **não** é enviado na criação. `acs` (Agente Comunitário de Saúde) é uma lista fixa de 14 pessoas, **obrigatória** em `POST /apacs` — não deriva de nenhum outro endpoint, é hardcoded em `APAC_ACS` mesmo. Todos já modelados em `src/lib/apac.ts` como `const object` + union type (`erasableSyntaxOnly` proíbe `enum`). `procedure` voltou a ser um enum — a API garante isso com um enum Prisma (`Procedures`) interno, agora um catálogo grande (~47 procedimentos) em vez dos antigos `EXAME`/`CIRURGIA`. **A chave que viaja no `POST`/`PATCH` é `SCREAMING_SNAKE_CASE` sem acento** (ex.: `CONSULTA_EM_DERMATOLOGIA_ADULTO`), **não** o texto formatado — enviar o texto bonito (`"CONSULTA EM DERMATOLOGIA - ADULTO"`) quebra com `500`/`PrismaClientValidationError`, já aconteceu em produção. Modelado em `src/lib/apac.ts`: `APAC_PROCEDURE`/`ApacProcedure` (chave = valor, como `APAC_ACS`), `APAC_PROCEDURE_LABEL` (chave → texto formatado só para exibição) e `APAC_PROCEDURE_OPTIONS` (`{ value, label }[]`, formato que o `Combobox` do Base UI reconhece automaticamente para busca/exibição). Hardcoded, não deriva de endpoint, mesmo caso de `APAC_ACS`. Grande demais para `Select`: nos formulários (`ApacFormRequestStep`, `ApacEditDialog`) é um `Combobox` com busca (`src/components/ui/combobox.tsx`) usando `APAC_PROCEDURE_OPTIONS` — **nunca envie o `label` como valor**, sempre o `value` (a chave do enum). Telas que só exibem (`ApacsTableCard`, `ApacDetailsDialog`, `ApacFormReviewStep`) usam `APAC_PROCEDURE_LABEL[apac.procedure]`, nunca o valor cru.
